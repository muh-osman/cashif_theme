...
try{if(searchValue!==""){spinner.style.display="block";const response=await fetch("https://cashif.online/back-end/public/api/car-models/search",{method:"POST",headers:{"Content-Type":"application/json",},body:JSON.stringify({search:searchValue}),});spinner.style.display="none";if(!response.ok){throw new Error("Failed to fetch data")}
...
button.innerHTML=" جاري البحث...";try{const response=await fetch(`https://cashif.online/back-end/public/api/get-prices-by-model-and-year?car_model_id=${clickedButtonId}&year_id=${yearValue}`,{method:"GET",headers:{"Content-Type":"application/json",},});if(!response.ok){button.innerHTML="أسعار الخدمة";throw new Error("Failed to fetch data from the new API")}
newData=await response.json();button.innerHTML="أسعار الخدمة";const engainPrice=document.getElementById("engain-price");engainPrice.textContent=`${
    ...
    } ريال`;const overlay=document.getElementById("overlay");overlay.style.padding="16px";overlay.style.gridTemplateRows="1fr";setTimeout(()=>{overlay.scrollIntoView({behavior:"smooth",block:"start",inline:"nearest",})},100)}catch(error){console.error("Error:",error);button.innerHTML="أسعار الخدمة"}};const planeOneBtn=document.getElementById("plane-one");const planeTwoBtn=document.getElementById("plane-two");const planeThreeBtn=document.getElementById("plane-three");const palneA=()=>{const url=`https://wa.me/966536181188?text=طلب خدمة فحص سيارة موديل "${clickedButtonTitle}" باقة "المحركات" - ${
    newData[0].prices[2].price * (1).toFixed(2)
