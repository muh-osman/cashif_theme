...
confirmRedeemeBtn.addEventListener("click", function () {
  ...
  if (inputValue === "") {
    alert("Please enter a redemption value");
  ...
  if (!Number.isInteger(numberValue)) {
    alert("Please enter a whole number without fractions");
    redeemeValueInput.focus();
    return;
  }
  ...
  if (numberValue > points) {
    alert(`You cannot redeem more points than your balance. Your current balance: ${points} points`);
    redeemeValueInput.focus();
    return;
  }

  if (numberValue > total) {
    alert(`You cannot redeem more points than the order value. Current order value: ${total} Riyals`);
    redeemeValueInput.focus();
    return;
  }

  if (numberValue === total) {
    alert(`You cannot redeem your points for the full order value. The minimum order amount is 100 Riyals`);
    redeemeValueInput.focus();
    return;
  }

  if (numberValue > total - 100) {
    alert(`You cannot redeem your points for the full invoice amount. The minimum order is 100 Riyals`);
    redeemeValueInput.focus();
    return;
  }
...
function updateTotal() {
  ...
  total = Math.floor(subtotal + videoPrice + summaryPrice + explainReport - discountAmount - redeemeAmoumntValue);

  if (total < 100) {
    alert("Minimum order value is 100 riyals.");
    location.reload();
    return false;
  }
